...
namespace GasScatteringCrossSection
{
   ...
   GasScatteringCrossSection::GasScatteringCrossSection(const ElementT& elm) : RandomizedScatterT("Edgerton gas cross-section", REFERENCE), mElement(elm), mElastic(*ScreenedRutherfordScatteringAngle::mScatter[elm.getAtomicNumber()])
   {
   }
